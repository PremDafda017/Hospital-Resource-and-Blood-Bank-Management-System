...
export default function HomePage() {
  ...
  const handleProtectedAction = () => {
    setMobileMenuOpen(false);
    if (!isSignedIn) {
      setLoginModalMessage("Please login first to access this feature.");
  ...
  const handleDonorRegistration = (e) => {
    ...
    setTimeout(() => setDonorSuccess(false), 6000);
  };
    ...
    <div style={{ fontFamily:FONT, color:SLATE, overflowX:"hidden", background: theme === "dark" ? "#0F172A" : WHITE, minHeight: "100vh" }}>
      ...
      <nav style={{
        ...
        <div className={`mobile-menu${mobileMenuOpen ? " open" : ""}`} style={{
          ...
          {isSignedIn && userProfile ? (
            <>
              <button onClick={() => { setMobileMenuOpen(false); navigate("/dashboard"); }} style={{ display:"flex", alignItems:"center", gap:12, padding:"14px 20px", color: navScrolled ? SLATE_MD : "rgba(255,255,255,0.9)", fontWeight:600, fontSize:"0.95rem", background:"none", border:"none", textAlign:"left", cursor:"pointer" }}>
